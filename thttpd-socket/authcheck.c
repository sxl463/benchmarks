/*
Sensitive partition (with old index):
V-SET:    88    89    91
Replicated functions (with old index):
V-SET:    36    43    44    58    59    60    61    70    71    95    96    135    136
Static boundary: (2.2,5.0,0.0,1.0)
E-SET:    (69, 88)    (89, 90)
Effective boundary: (2.2,5.0,0.0,1.0)
E-SET:    (69, 88)    (89, 90)
*/
...
static int auth_check2( httpd_conn* hc, char* dirname  ); // 89
static void send_authenticate( httpd_conn* hc, char* realm ); // 91

char* 
httpd_ntoa( httpd_sockaddr* saP ); // 36

void 
httpd_realloc_str( char** strP, size_t* maxsizeP, size_t size ); // 43

void 
httpd_send_err( httpd_conn* hc, int status, char* title, char* extraheads, char* form, char* arg ); // 44

static size_t 
sockaddr_len( httpd_sockaddr* saP ); // 58
static int
my_snprintf( char* str, size_t size, const char* format, ... ); // 59

static int
send_err_file( httpd_conn* hc, int status, char* title, char* extraheads, char* filename ); // 60

static void
send_response( httpd_conn* hc, int status, char* title, char* extraheads, char* form, char* arg ); // 61

static void
send_mime( httpd_conn* hc, int status, char* title, char* encodings, char* extraheads, char* type, off_t length, time_t mod ); // 70

static void
add_response( httpd_conn* hc, char* str ); // 71

static void
send_response_tail( httpd_conn* hc ); // 95

static void
defang( char* str, char* dfstr, int dfsize ); // 96

int
match( const char* pattern, const char* string ); // 135
